...
const HomeScreen: React.FC<IHomeScreen> = ({ navigation, route }: HomeProps) => {
    ...
    const { onPress, getTotalIncome, getTotalExpense, saving } = useHomeScreenViewController({ navigation, route });
    return (
        <HomeScreenView onClick={onPress} totalIncome={getTotalIncome} totalExpense={getTotalExpense} totalSaving={saving} />
    )
}
